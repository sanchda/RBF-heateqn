function Unew = heateqn(U0, x, h, M, alpha, lambda, eps, surfeps)
...
% TODO: lit citation
    ...
    % Assuming that U0 is a spherical harmonic, give the analytical
    % solution at time t
    Uan = @(t) exp(alpha*lambda*t)*U0;
...
%         
%        name = sprintf('../lapout/image_%03d.png',i-1);
%        print('-dpng',name);
%        close all force;
%        close all hidden;
%        
...
%         drawnow
%         
%         name = sprintf('../output5/imageUNSCALED_%03d.png',i-1);
%         print('-dpng',name);
%        close all force;
%        close all hidden;
        Unew(i) = sum(abs(U - Uan(h*i)))/N;
